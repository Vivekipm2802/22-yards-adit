...
// 12 zones, evenly distributed every 30°. Angle is measured from 12 o'clock
// clockwise (0° = straight past the bowler). Names use RHB convention.
const WAGON_ZONES = [
  { key: 'STRAIGHT',      label: 'Straight',     angle: 0,   side: 'O' },
  { key: 'MID_OFF',       label: 'Mid Off',      angle: 30,  side: 'O' },
  { key: 'EXTRA_COVER',   label: 'Extra Cover',  angle: 60,  side: 'O' },
  { key: 'COVER',         label: 'Cover',        angle: 90,  side: 'O' },
  { key: 'POINT',         label: 'Point',        angle: 120, side: 'O' },
  { key: 'THIRD_MAN',     label: 'Third Man',    angle: 150, side: 'O' },
  { key: 'FINE_LEG',      label: 'Fine Leg',     angle: 180, side: 'L' },
  { key: 'LONG_LEG',      label: 'Long Leg',     angle: 210, side: 'L' },
  { key: 'SQUARE_LEG',    label: 'Square Leg',   angle: 240, side: 'L' },
  { key: 'MID_WICKET',    label: 'Mid Wicket',   angle: 270, side: 'L' },
  { key: 'MID_ON',        label: 'Mid On',       angle: 300, side: 'L' },
  { key: 'LONG_ON',       label: 'Long On',      angle: 330, side: 'L' },
];
...
const runColor = (r: number): string => {
  ...
  if (r >= 4) return '#39FF14';      // four — neon green
  if (r === 3) return '#FFD600';     // three — gold
  if (r === 2) return '#00F0FF';     // two — cyan
  if (r === 1) return '#FFFFFF';     // single — white
  return '#666666';                   // dot
};
...
const MatchCenter: React.FC<{ onBack: () => void; onNavigate?: (page: string) => void }> = ({ onBack, onNavigate }) => {
  ...
  // ---- Wagon Wheel ---------------------------------------------------------
  const [wagonPicker, setWagonPicker] = useState<{
    open: boolean;
    ballId: string | null;
    runs: number;
    batsmanName: string;
    zone: string | null;
    step: 1 | 2;
  }>({ open: false, ballId: null, runs: 0, batsmanName: '', zone: null, step: 1 });
  const [wagonFilter, setWagonFilter] = useState<'ALL' | 'A' | 'B' | string>('ALL'); // ALL/TeamID/playerID
  ...
  const commitBall = (runs: number, extra?: string, isWicket?: boolean, wicketType?: string, fielderId?: PlayerID) => {
    // Capture values needed for the wagon wheel picker before the state updater runs
    let pickerPayload: { ballId: string; runs: number; batsmanName: string } | null = null;
    setMatch(m => {
      ...
      // Capture info for the wagon wheel picker (shown after state updates if enabled + scoring ball)
      if (m.config.wagonWheel !== false && runs > 0 && !isWicket) {
        const strikerObj = (m.teams[battingTeamKey]?.squad || []).find(p => p.id === m.crease.strikerId);
        pickerPayload = {
          ballId: ballEvent.ballId,
          runs,
    ...
    // Open wagon wheel picker if applicable (runs after state updates flush)
    if (pickerPayload) {
      setTimeout(() => {
        setWagonPicker({
          open: true,
          ballId: pickerPayload!.ballId,
          runs: pickerPayload!.runs,
          batsmanName: pickerPayload!.batsmanName,
          zone: null,
          step: 1,
        });
      }, 80);
  ...
  // Tag a previously-committed ball with wagon wheel zone + shot type
  const tagBallWithShot = (ballId: string, zone: string | null, shotType: string | null) => {
    setMatch(m => ({
      ...m,
      history: (m.history || []).map(h =>
        h.ballId === ballId ? { ...h, shotZone: zone || undefined, shotType: shotType || undefined } : h
      ),
    }));
  };

  const closeWagonPicker = () => setWagonPicker({ open: false, ballId: null, runs: 0, batsmanName: '', zone: null, step: 1 });

  const handleWagonZonePick = (zoneKey: string) => {
    setWagonPicker(p => ({ ...p, zone: zoneKey, step: 2 }));
  };

  const handleWagonShotPick = (shotKey: string) => {
    if (wagonPicker.ballId && wagonPicker.zone) {
      tagBallWithShot(wagonPicker.ballId, wagonPicker.zone, shotKey);
    }
    if (navigator.vibrate) navigator.vibrate(30);
    closeWagonPicker();
  };

  const handleWagonSkip = () => {
    closeWagonPicker();
  };

  const toggleWagonWheel = () => {
    setMatch(m => {
      const current = m.config.wagonWheel !== false;
      return { ...m, config: { ...m.config, wagonWheel: !current } };
    });
  };
  ...
  return (
    <div className="h-full w-full bg-[#050505] text-white flex flex-col overflow-hidden relative font-sans max-h-[100dvh]">
      ...
      <div className="main-header h-14 flex items-center px-6 border-b border-white/5 bg-black/50 backdrop-blur-md z-[100] shrink-0">
        ...
        {status === 'LIVE' && (
          <div className="flex items-center space-x-1">
            ...
            {match.config.wagonWheel !== false && (
              <button onClick={() => setShowWagonWheelModal(true)} className="p-2 text-[#BC13FE] hover:bg-white/5 rounded-full transition-all" title="Wagon Wheel">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="2" x2="12" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/><line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/><line x1="19.07" y1="4.93" x2="4.93" y2="19.07"/></svg>
              </button>
            )}
        ...
        {status === 'SUMMARY' && match.config.wagonWheel !== false && (
          <button onClick={() => setShowWagonWheelModal(true)} className="p-2 text-[#BC13FE] hover:bg-white/5 rounded-full transition-all" title="Wagon Wheel">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="2" x2="12" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/><line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/><line x1="19.07" y1="4.93" x2="4.93" y2="19.07"/></svg>
          </button>
        )}
        ...
        {showMatchSettings && (
          ...
          >
            ...
            >
              ...
              {/* WAGON WHEEL — Toggle */}
              <div className="w-full p-4 rounded-[20px] bg-gradient-to-br from-[#BC13FE]/10 to-[#00F0FF]/5 border border-[#BC13FE]/25 flex items-center justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-10 h-10 rounded-[12px] bg-[#BC13FE]/15 border border-[#BC13FE]/30 flex items-center justify-center flex-shrink-0">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#BC13FE" strokeWidth="2">
                  ...
                  <div className="min-w-0">
                    <div className="text-[12px] font-black uppercase tracking-wider text-white">Wagon Wheel</div>
                    <div className="text-[9px] text-white/50 font-bold uppercase tracking-wider">Tag zone & shot after every run</div>
                  </div>
                </div>
                <button
                  onClick={toggleWagonWheel}
                  className={`relative w-[52px] h-[30px] rounded-full transition-all duration-300 flex-shrink-0 ${match.config.wagonWheel !== false ? 'bg-gradient-to-r from-[#BC13FE] to-[#00F0FF] shadow-[0_0_16px_rgba(188,19,254,0.5)]' : 'bg-white/10 border border-white/15'}`}
                  aria-label="Toggle wagon wheel"
                >
                  <motion.div
                    animate={{ x: match.config.wagonWheel !== false ? 24 : 2 }}
      ...
      {/* WAGON WHEEL PICKER MODAL — opens after every scoring delivery */}
      <AnimatePresence>
        {wagonPicker.open && (
          <motion.div
            initial={{ opacity: 0 }}
            ...
            className="fixed inset-0 z-[9800] bg-black/92 backdrop-blur-md flex items-center justify-center p-4"
          >
            <motion.div
              ...
              className="relative w-full max-w-md rounded-[36px] overflow-hidden bg-gradient-to-br from-[#08080D] via-[#0B0B14] to-[#050509] border border-[#39FF14]/20 shadow-[0_0_80px_rgba(57,255,20,0.12)]"
            >
              {/* Glow accents */}
              <motion.div
                aria-hidden
                className="absolute -top-20 -right-24 w-56 h-56 rounded-full blur-[80px] pointer-events-none"
              ...
              <motion.div
                aria-hidden
                className="absolute -bottom-16 -left-20 w-48 h-48 rounded-full blur-[80px] pointer-events-none"
                style={{ background: 'radial-gradient(circle, rgba(0,240,255,0.16), transparent 70%)' }}
                animate={{ opacity: [0.3, 0.6, 0.3] }}
              ...
              <div className="relative p-5 flex items-center justify-between border-b border-white/5">
                <div className="flex items-center gap-3">
                  <motion.div
                    animate={{ rotate: [0, 8, -8, 0] }}
                    transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}
                    className="w-10 h-10 rounded-full bg-gradient-to-br from-[#39FF14]/20 to-[#00F0FF]/10 border border-[#39FF14]/30 flex items-center justify-center"
                  >
                    <Crosshair size={16} className="text-[#39FF14]" />
                  </motion.div>
                  <div>
                    <h3 className="font-heading text-base uppercase italic text-[#39FF14]">Wagon Wheel</h3>
                    <p className="text-[8px] text-white/40 uppercase tracking-[0.22em]">
                      {wagonPicker.step === 1 ? 'Step 1 · Tap the zone' : 'Step 2 · Shot played'}
                    </p>
                  </div>
                </div>
                <button
                  onClick={handleWagonSkip}
                  className="px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-[0.22em] text-white/40 hover:text-white hover:bg-white/5 transition-all"
                >
                  Skip
                </button>
              </div>
              ...
              <div className="relative px-5 pt-4">
                <div className="flex items-center justify-between p-3 rounded-[16px] bg-white/[0.03] border border-white/10">
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[#FFD600] to-[#FF6D00] flex items-center justify-center text-[9px] font-black text-black flex-shrink-0">
                      {wagonPicker.batsmanName.charAt(0)}
                    </div>
                    <p className="text-[11px] font-black text-white truncate">{wagonPicker.batsmanName}</p>
                  </div>
                  <div
                    className="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-[0.2em]"
                    style={{
                      background: `${runColor(wagonPicker.runs)}22`,
                      border: `1px solid ${runColor(wagonPicker.runs)}55`,
                      color: runColor(wagonPicker.runs),
                    }}
                  >
                    {wagonPicker.runs === 4 ? 'FOUR' : wagonPicker.runs === 6 ? 'SIX' : `${wagonPicker.runs} RUN${wagonPicker.runs > 1 ? 'S' : ''}`}
                  </div>
                </div>
              </div>

              {/* Content: zone picker OR shot picker */}
              <div className="relative p-5">
                <AnimatePresence mode="wait">
                  {wagonPicker.step === 1 ? (
                    ...
                    >
                      {/* Interactive SVG zone picker */}
                      <div className="relative w-[280px] h-[280px]">
                        <svg viewBox="-160 -160 320 320" className="w-full h-full">
                          {/* Outer field (deep) */}
                          <defs>
                            <radialGradient id="fieldGradOuter" cx="50%" cy="50%" r="50%">
                              <stop offset="0%" stopColor="#0d3e1a" />
                              <stop offset="70%" stopColor="#082814" />
                              <stop offset="100%" stopColor="#041208" />
                            </radialGradient>
                            <radialGradient id="fieldGradInner" cx="50%" cy="50%" r="50%">
                              <stop offset="0%" stopColor="#1a5c2b" />
                              <stop offset="100%" stopColor="#0d3e1a" />
                            </radialGradient>
                          </defs>
                          <circle r="150" fill="url(#fieldGradOuter)" stroke="#39FF14" strokeOpacity="0.35" strokeWidth="1.5" />
                          <circle r="85" fill="url(#fieldGradInner)" stroke="#39FF14" strokeOpacity="0.22" strokeWidth="1" strokeDasharray="3 3" />

                          {/* Pitch */}
                          <rect x="-8" y="-30" width="16" height="60" fill="#8B6B3A" stroke="#c9a769" strokeWidth="0.8" rx="1" />
                          <line x1="-8" y1="-20" x2="8" y2="-20" stroke="#fff" strokeWidth="0.6" />
                          <line x1="-8" y1="20" x2="8" y2="20" stroke="#fff" strokeWidth="0.6" />

                          {/* 12 zone wedges */}
                          {WAGON_ZONES.map((z) => {
                            const halfArc = 15; // degrees (30° slice, ±15° each side)
                            const a1 = (z.angle - halfArc - 90) * Math.PI / 180;
                            const a2 = (z.angle + halfArc - 90) * Math.PI / 180;
                            const r = 149;
                            const x1 = Math.cos(a1) * r;
                            const y1 = Math.sin(a1) * r;
                            const x2 = Math.cos(a2) * r;
                            const y2 = Math.sin(a2) * r;
                            const path = `M 0 0 L ${x1.toFixed(2)} ${y1.toFixed(2)} A ${r} ${r} 0 0 1 ${x2.toFixed(2)} ${y2.toFixed(2)} Z`;
                            // Label position (midway, at radius 115)
                            const labelA = (z.angle - 90) * Math.PI / 180;
                            const lx = Math.cos(labelA) * 115;
                            const ly = Math.sin(labelA) * 115;
                            const isOff = z.side === 'O';
                            return (
                              <g key={z.key}>
                                <motion.path
                                  d={path}
                                  fill={isOff ? 'rgba(0, 240, 255, 0.05)' : 'rgba(255, 109, 0, 0.05)'}
                                  stroke="rgba(255,255,255,0.12)"
                                  strokeWidth="0.8"
                                  whileHover={{
                                    fill: isOff ? 'rgba(0, 240, 255, 0.22)' : 'rgba(255, 109, 0, 0.22)',
                                  }}
                                  whileTap={{ scale: 0.97 }}
                                  style={{ cursor: 'pointer', transformOrigin: 'center' }}
                                  onClick={() => handleWagonZonePick(z.key)}
                                />
                                <text
                                  x={lx}
                                  y={ly}
                                  fill={isOff ? '#00F0FF' : '#FF9E45'}
                                  fontSize="9"
                                  fontWeight="900"
                                  textAnchor="middle"
                          ...
                          {/* Center dot / batsman */}
                          <circle r="4" fill="#FFD600" stroke="#fff" strokeWidth="0.8" />
                        </svg>
                        {/* Off/Leg legend */}
                        <div className="absolute -bottom-1 left-0 right-0 flex justify-center gap-5 text-[8px] font-black uppercase tracking-[0.2em]">
                          <span className="text-[#00F0FF]">Off Side</span>
                          <span className="text-white/30">·</span>
                          <span className="text-[#FF9E45]">Leg Side</span>
                        </div>
                      </div>
                    </motion.div>
                  ) : (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-center gap-2">
                        <button
                          onClick={() => setWagonPicker(p => ({ ...p, step: 1, zone: null }))}
                          className="p-1.5 rounded-full bg-white/5 border border-white/10 text-white/60 hover:text-white hover:bg-white/10 transition-all"
                        >
                          <ChevronLeft size={14} />
                        </button>
                        <div className="px-3 py-1 rounded-full bg-[#39FF14]/10 border border-[#39FF14]/30 text-[#39FF14] text-[10px] font-black uppercase tracking-[0.2em]">
                          {WAGON_ZONES.find(z => z.key === wagonPicker.zone)?.label || '—'}
                        </div>
                      </div>
      ...
      <AnimatePresence>
        {showWagonWheelModal && (
          <motion.div
            ...
            className="fixed inset-0 z-[5200] bg-black/95 backdrop-blur-sm flex items-center justify-center p-4"
          >
            <motion.div
              ...
              transition={{ type: 'spring', damping: 26, stiffness: 320 }}
              onClick={(e) => e.stopPropagation()}
              className="relative w-full max-w-lg max-h-[92vh] overflow-y-auto rounded-[36px] bg-gradient-to-br from-[#08080D] via-[#0B0B14] to-[#050509] border border-[#39FF14]/20 shadow-[0_0_80px_rgba(57,255,20,0.1)]"
            >
              {/* Header */}
              <div className="sticky top-0 z-10 p-5 bg-gradient-to-b from-[#08080D] via-[#08080D]/95 to-transparent flex items-center justify-between border-b border-white/5">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#39FF14]/20 to-[#00F0FF]/10 border border-[#39FF14]/30 flex items-center justify-center">
                    <Crosshair size={16} className="text-[#39FF14]" />
                  </div>
                  <div>
                    <h3 className="font-heading text-lg uppercase italic text-[#39FF14]">Wagon Wheel</h3>
                    <p className="text-[9px] text-white/40 uppercase tracking-[0.22em]">Shot placement map</p>
                  </div>
                </div>
                <button onClick={() => setShowWagonWheelModal(false)} className="p-2 text-white/40 hover:text-white rounded-full hover:bg-white/5 transition-all">
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="px-5 pt-4">
                <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1">
                  {[
                    ...
                    { k: 'A', label: match.teams.teamA.name || 'Team A' },
                    { k: 'B', label: match.teams.teamB.name || 'Team B' },
                  ].map(t => (
                    <button
                      key={t.k}
                      onClick={() => setWagonFilter(t.k)}
                      className={`flex-shrink-0 px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-[0.18em] transition-all border ${
                        wagonFilter === t.k
              ...
              <div className="p-5 flex flex-col items-center">
                {(() => {
                  const allBalls = (match.history || []).filter((b: any) => b.shotZone && !b.isWicket && b.runsScored > 0);
                  const filtered = allBalls.filter((b: any) => {
                    if (wagonFilter === 'ALL') return true;
                    if (wagonFilter === 'A' || wagonFilter === 'B') return b.teamId === wagonFilter;
                    return b.strikerId === wagonFilter;
                  });
                  return (
                    <div className="relative w-full max-w-[360px]">
                      <svg viewBox="-160 -160 320 320" className="w-full">
                        <defs>
                          <radialGradient id="wwOuter" cx="50%" cy="50%" r="50%">
                            <stop offset="0%" stopColor="#0d3e1a" />
                            <stop offset="70%" stopColor="#082814" />
                            <stop offset="100%" stopColor="#041208" />
                          </radialGradient>
                          <radialGradient id="wwInner" cx="50%" cy="50%" r="50%">
                            <stop offset="0%" stopColor="#1a5c2b" />
                            <stop offset="100%" stopColor="#0d3e1a" />
                          </radialGradient>
                        </defs>
                        {/* Field */}
                        <circle r="150" fill="url(#wwOuter)" stroke="#39FF14" strokeOpacity="0.35" strokeWidth="1.5" />
                        <circle r="85" fill="url(#wwInner)" stroke="#39FF14" strokeOpacity="0.22" strokeWidth="1" strokeDasharray="3 3" />
                        {/* 30 yd reference arcs */}
                        {[0, 90, 180, 270].map(ang => {
                          const rad = (ang - 90) * Math.PI / 180;
                          return <line key={ang} x1={0} y1={0} x2={Math.cos(rad) * 150} y2={Math.sin(rad) * 150} stroke="rgba(255,255,255,0.06)" strokeWidth="0.5" />;
                        })}
                        {/* Pitch */}
                        <rect x="-8" y="-30" width="16" height="60" fill="#8B6B3A" stroke="#c9a769" strokeWidth="0.8" rx="1" />
                        <line x1="-8" y1="-20" x2="8" y2="-20" stroke="#fff" strokeWidth="0.6" />
                        ...
                        {filtered.map((b: any, i: number) => {
                          const z = WAGON_ZONES.find(zz => zz.key === b.shotZone);
                          if (!z) return null;
                          const a = (z.angle - 90) * Math.PI / 180;
                          // Length: 4s and 6s go to boundary, smaller runs stay shorter
                          const len = b.runsScored >= 6 ? 148 : b.runsScored >= 4 ? 142 : b.runsScored === 3 ? 120 : b.runsScored === 2 ? 100 : 82;
                          const x2 = Math.cos(a) * len;
                          const y2 = Math.sin(a) * len;
                          const col = runColor(b.runsScored);
                          // Slight angular jitter so overlapping zones don't all stack on one line
                          const jitter = ((i % 5) - 2) * 2.5;
                          const jRad = ((z.angle + jitter) - 90) * Math.PI / 180;
                          const jx2 = Math.cos(jRad) * len;
                          const jy2 = Math.sin(jRad) * len;
                          return (
                            <motion.line
                              key={b.ballId}
                              x1={0} y1={0}
                              x2={jx2} y2={jy2}
                              stroke={col}
                              strokeWidth={b.runsScored >= 4 ? 1.8 : 1.2}
                              strokeLinecap="round"
                              initial={{ pathLength: 0, opacity: 0 }}
                              animate={{ pathLength: 1, opacity: 0.95 }}
                              transition={{ duration: 0.5, delay: i * 0.02 }}
                              style={{ filter: b.runsScored >= 4 ? `drop-shadow(0 0 3px ${col})` : 'none' }}
                            />
                          );
                        })}
                      ...
                      {filtered.length === 0 && (
                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                          <p className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em] text-center max-w-[200px]">No shot data yet. Tagged shots will appear here.</p>
                        </div>
                      )}
                    </div>
                  );
                ...
                <div className="mt-4 flex items-center justify-center gap-3 flex-wrap">
                  {[
                    ...
                    <div key={x.l} className="flex items-center gap-1.5">
                      <div className="w-4 h-[2px] rounded-full" style={{ background: x.c, boxShadow: `0 0 6px ${x.c}` }} />
                      <span className="text-[9px] font-black text-white/70 uppercase">{x.l}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="px-5 pb-6">
                {(() => {
                  ...
                  const byZone: Record<string, { runs: number; count: number }> = {};
                  filtered.forEach((b: any) => {
                    if (!byZone[b.shotZone]) byZone[b.shotZone] = { runs: 0, count: 0 };
                    byZone[b.shotZone].runs += b.runsScored;
                    byZone[b.shotZone].count += 1;
                  });
                  const sorted = Object.entries(byZone).sort((a, b) => b[1].runs - a[1].runs).slice(0, 4);
                  const totalRuns = filtered.reduce((s: number, b: any) => s + b.runsScored, 0);
                  return (
                    <div className="space-y-2">
                      <p className="text-[9px] font-black text-white/40 uppercase tracking-[0.22em]">Top Scoring Zones</p>
                      <div className="space-y-1.5">
                        {sorted.map(([zKey, d]) => {
                          const z = WAGON_ZONES.find(zz => zz.key === zKey);
                          const pct = totalRuns > 0 ? (d.runs / totalRuns) * 100 : 0;
                          return (
                            <div key={zKey} className="flex items-center gap-3">
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between mb-1">
                                  <span className="text-[10px] font-black text-white/80 uppercase tracking-wide truncate">{z?.label || zKey}</span>
                                  <span className="text-[10px] font-black text-[#39FF14]">{d.runs} <span className="text-white/30">·</span> {d.count} ball{d.count === 1 ? '' : 's'}</span>
                                </div>
